                            ...
                            if self.checkForEnd():
                                ...
                                if game.board.score[game.board.currentPlayer] > game.board.score[
                                    ...
                                    game.save_winner_to_collector(winner, self.white_capture, self.black_capture,
                                                                  self.white_placed, self.black_placed, score)
                                    s = str(number_of_games) + ':' + str(
                                        winner) + ':' + str(score) +':'+"No winner. Barrier case."+':'+'Total no_moves: {}'.format(num_of_moves)
                                    with open("game_record.txt",'a+') as f:
                                        f.write(s+"\n")
                                   ...
                                   # end = QMessageBox.information(game, "End", "No winner. Barrier case.")
                                    ...
                                    score = game.board.score
                                    game.save_winner_to_collector(winner,self.white_capture,self.black_capture,self.white_placed,self.black_placed,score)
                                ...
                                if self.isStuck((self.color[((game.board.currentPlayer) + 1) % 2])):
                                    ...
                                    game.save_winner_to_collector(10,self.white_capture,self.black_capture,self.white_placed,self.black_placed,game.board.score)
                                    s = str(number_of_games) + ':'+str(game.board.score)+":" + "Player is stuck"+':'+'Total no_moves: {}'.format(num_of_moves)
                                    with open("game_record.txt", 'a+') as f:
                                        f.write(s+"\n")
...
class GameWindow(QMainWindow):
    ...
    def save_winner_to_collector(self,winner,captured_white,captured_black,white_placed,black_placed,score):
        ...
        assert(len(white_reward)==len(captured_white)+white_placed and len(black_reward) == len(captured_black)+black_placed)
        print("---FINAL REWARD---")
        print("FOR WHITE: {}".format(white_reward))
        print("LENGTH : {}".format(len(white_reward)))
        print("FOR BLACK: {}".format(black_reward))
        print("Lenght : {}".format(len(black_reward)))
        '''
        if score_black > score_white:
            self.collector_white.complete_episode(reward=-1)
            self.collector_black.complete_episode(reward=1)
        elif score_white > score_black:
            self.collector_white.complete_episode(reward=1)
            self.collector_black.complete_episode(reward=-1)
        else:
            self.collector_white.complete_episode(reward=0.5)
            self.collector_black.complete_episode(reward=0.5)
        ...
        if number_of_games == self.train_samples:
            ...
            if os.path.isdir('experience/experience{}'.format(self.instance)) == False:
                try:
                    os.mkdir('experience/experience{}'.format(self.instance))
                except OSError:
                    print("ERROR CREATING DIRECTORY: experience/experience{}".format(self.instance))

            filename = 'experience/experience{}/seegaexperience{}.hdf5'.format(self.instance,self.instance)
    ...
    def newGame(self,n_g):
        ...
        exp_filename = 'experience/seegaexperience_{}_{}.hdf5'.format(number_of_games,num_of_moves)
        print(exp_filename)
        num_of_moves = 0
        '''
...
if __name__ == "__main__":
    ...
    game = GameWindow(gameSize, agents,train_samples=int(args.train_samples),instance=int(args.instance), sleep_time=sleep_time, timeout=timeout)
