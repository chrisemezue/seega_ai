...
class IA(Player):
    ...
    def __init__(self, position, gameSize):
        ...
        if os.path.exists(agent_dir) != True:
            ...
            board_input = Input(shape=(self.encoder.board_height(), self.encoder.board_width(), self.encoder.layers_encode), name='board_input')

            conv1 = Conv2D(128, (2, 2), padding='valid',strides=1)(board_input)
            conv2 = Conv2D(128, (2, 2), padding='same', activation='relu')(conv1)
            #conv2 = Dropout(0.5)(conv2)
            conv3 = Conv2D(128, (2, 2), padding='same',activation='sigmoid')(conv2)
            ...
            policy_output = Dense(self.encoder.num_moves(),activation='softmax')(policy_hidden_layer)
