...
class RulesGame:
    ...
    def play(self, x, y):
        global number_of_games, score_white,score_black
                    ...
                    if (self.canPlayHere(x, y)):
                        ...
                        if self.origin is not None and (x, y) in self.getRealsMoves(self.origin[0], self.origin[1]):
                            ...
                            if self.checkForEnd():
                                ...
                                if self.isStuck((self.color[((game.board.currentPlayer) + 1) % 2])):
                                    game.setStatusTip("Player is stuck")
                                    #score_black = score_black + int(score[0])
                                    #score_white = score_white + int(score[1])
                                    number_of_games += 1
                                    # game.newGame()
                                    game.gameOneGoing = False
                                else:
...
class GameWindow(QMainWindow):
    ...
    def __init__(self, gameSize, players,train_samples,  timeout=.50, sleep_time=.500, parent=None, ):
        super(GameWindow, self).__init__(parent)
    ...
    def newGame(self, n_g):
        global num_of_moves
        global number_of_games, score_black,score_white
        global num_train
        print("Number of games: {}".format(number_of_games))
        print("Train samples: {}".format(self.train_samples))
        if number_of_games == self.train_samples:
            number_of_games = 0
            print("Game Ended")
            print("{} :  {}".format(self.player1.getName(),score_black))
            print("{} :  {}".format(self.player2.getName(), score_white))

            s = str(score_black) + ':' + str(score_white)
            with open("agent_test_result.txt", 'a+') as f:
                f.write(s + ":")
...
if __name__ == "__main__":
    ...
    game = GameWindow(gameSize, agents, train_samples=int(args.train_samples),sleep_time=sleep_time, timeout=timeout)
