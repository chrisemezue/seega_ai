...
#Adding of environmenrt for RL by Chris EMEZUE
...
'''-----Here we create the SeegaEnv'''

pieces_to_ids={'black':1,'white':-1,'None':0}
...
def make_random_policy(np_random):
	def random_policy(state):
		opp_player = -1
		moves = SeegaEnv.get_possible_moves(state, opp_player)
		# No moves left
		if len(moves) == 0:
			return 'resign'
		else:
...
""" 
    SEEGA GYM ENVIRONMENT CLASS
    --------------------------- 
"""


class SeegaEnv(gym.Env):
    pieces_values = {'black': 1, 'white': -1}
    ids_to_pieces = {v: k for k, v in pieces_to_ids.items()}
    WHITE = -1
    BLACK = 1

    def __init__(self, agents,player_color=1, opponent="random", log=True):
        #self.moves_max = 149
        self.log = log
        self.agents = agents
        # One action (for each board position) x (no. of pieces)
        self.observation_space = spaces.Box(-1, 1, (gameSize, gameSize))  # board 5x5
        self.action_space = spaces.Discrete(25 * 3) #This is the possible number of all actions in the board

        self.player = self.agents[0]  # define player # TODO: implement
    ...
    def _step(self, action):
        ...
        #
            ...
            opp_action = SeegaEnv.move_to_actions(opp_move)
    ...
    def player_move(self, player, state, action, render=False, render_msg='Player'):
        ...
        """
        # Resign
        if SeegaEnv.has_resigned(action):
            return state, -100, True
        # Play
        move = SeegaEnv.action_to_move(action, player) #This is where the play action for the player should be called
        new_state, prev_piece, reward = SeegaEnv.next_state(
            copy(state), move, player)
        # Keep track of movements
        piece_id = move['piece_id']
        if abs(piece_id) == SeegaEnv.CASTLE_MOVE_ID:
        ...
        self.saved_states = SeegaEnv.encode_current_state(state, self.saved_states)
        ...
        ###Render
        if render:
            SeegaEnv.render_moves(state, move['piece_id'], [move], mode='human')
    ...
    def _reset(self,game):
        ...
        SeegaEnv.ids_to_pieces = {v: k for k, v in pieces_to_ids.items()}
        # vars
        self.state = {}
        self.done = False
        self.current_player_in_game = 1
        self.saved_states = {}
        ...
        self.state['board'] = self.board.getListBoard()
    ...
    def render_board(state, mode='human', close=False):
        ...
        for i in range(7, -1, -1):
            ...
            for j in range(7, -1, -1):
                piece = SeegaEnv.ids_to_pieces[board[i, j]]
        ...
        for i in range(7, -1, -1):
            ...
            for j in range(7, -1, -1):
                ...
                if board[i, j] == piece_id:
                    ...
                    if piece == '.':
                        if piece_id == SeegaEnv.CASTLE_MOVE_ID:
                            outfile.write('0-0')
                        else:
                            outfile.write(' X ')
                    else:
    ...
    def encode_current_state(state, saved_states):
        ...
        for i in range(8):
            for j in range(8):
                ...
                piece_type = SeegaEnv.ids_to_pieces[piece_id][0].lower()
    ...
    def move_to_actions(move):
        ...
        """
        if move == 'resign':
            return SeegaEnv.resign_action()
        elif move['type'] == 'castling':
            return SeegaEnv.castle_move_to_action(move['castle'])
        else:
            piece_id = move['piece_id']
    ...
    def action_to_move(action, player):
        ...
        castles_pos = {
            1: {
                SeegaEnv.KING_CATSLE: [0, 1],
                SeegaEnv.QUEEN_CATSLE: [0, 5],
            },
            -1: {
                SeegaEnv.KING_CATSLE: [7, 1],
                SeegaEnv.QUEEN_CATSLE: [7, 5],
            }
        }
        ...
        if action in [t + 1, t + 2]:  # castling
            ...
            return {
                'piece_id': player * SeegaEnv.CASTLE_MOVE_ID,
                'pos': None,
                'new_pos': new_pos,
                'type': 'castling',
                'castle': castle_type
    ...
    def next_state(state, move, player):
        ...
        # check for castle
        # TODO
        if piece_id == player * SeegaEnv.CASTLE_MOVE_ID:
            return SeegaEnv.castle_action_to_state(
        ...
        piece_type = SeegaEnv.ids_to_pieces[prev_piece][0].lower()
        reward += SeegaEnv.pieces_values[piece_type]

        # check for pawn becoming a queen
        if SeegaEnv.ids_to_pieces[piece_id][0].lower() == 'p':
            if (player == 1 and new_pos[0] == 7):
                SeegaEnv.ids_to_pieces[piece_id] = 'Q'
                reward += 10
