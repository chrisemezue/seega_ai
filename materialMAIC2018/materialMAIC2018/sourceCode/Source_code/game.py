    ...
    def play(self,x,y):
        if game.gameOneGoing:
            if(self.step==0):
                ...
                if(self.seedOnBoard==24):
                    self.step=(self.step+1)%2
                    game.board.currentPlayer=(game.board.currentPlayer+1)%2
                    game.panel.updateCurrentPlayer()

                if self.seedOnBoard == 24 and self.isStuck(self.color[game.board.currentPlayer]):
                    game.board.currentPlayer = (game.board.currentPlayer+1)%2
                    game.panel.updateCurrentPlayer()
                ...
                if game.board.currentPlayer == 0:
                    ...
                    if(self.canPlayHere(x, y)):
                        ...
                        if self.origin is not None and (x,y) in self.getRealsMoves(self.origin[0], self.origin[1]):
                            ...
                            game.trace.add_action(game.board.currentPlayer, (self.origin, (x,y)), self.step, game.board.getListBoard(), game.board.score)
                            game.board.setDefaultColors()
                            print("Here")
                            ...
                            if self.checkForEnd():
                                if game.board.score[game.board.currentPlayer] > game.board.score[(game.board.currentPlayer + 1)%2]:
                                    ...
                                    end = QMessageBox.information(game, "End", f"Player {game.board.currentPlayer + 1} Win")
                                    ...
                                    end = QMessageBox.information(game, "End", f"Player {(game.board.currentPlayer + 1)%2 + 1} Win")
                                game.trace.winner = winner
                                game.trace.add_action(game.board.currentPlayer, (tempOrigin, (x,y)), self.step, game.board.getListBoard(), game.board.score)
                                game.saveGame()
    ...
    def mousePressEvent(self, ev):
        if ev.button() == 1:
            if self.active == True and game.gameOneGoing == True:
                ...
                #monboard1= QWidget()
                #a=[monboard1]
                #monboard2=copy.deepcopy(a)
                print(game.board.getListBoard())
...
if __name__=="__main__":
    ...
    gameSize=5
    game = GameWindow(gameSize, ["Player 1", "Player 2"])
